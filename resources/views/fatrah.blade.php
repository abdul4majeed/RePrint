      ...
      <div class="container">
        <div class="row">
          <div class="col-lg-12 mb-5" >
            ...
            <form action="{{route('exe_fatrah')}}" method="post" class="d-inline-block mb-2">
                {{csrf_field()}}
                <input type="hidden" name="card_id" value="{{$card->PaymentMethodId}}">
                <div class="card" style="width: 18rem;">
                    <img class="card-img-top" src="{{$card->ImageUrl}}" alt="{{$card->PaymentMethodEn}}">
                    <div class="card-body">
                      <h5 class="card-title">{{$card->PaymentMethodEn}}</h5>
                      <div>
                          <p class="card-text"><span>Price:</span><span>{{$card->TotalAmount - $card->ServiceCharge}}</span></p>
                          <p class="card-text"><span>Service Charges:</span>{{$card->ServiceCharge}}<span></span></p>
                          <p class="card-text"><span>Total Price:</span><span>{{$card->TotalAmount}}</span></p>
                      </div>
